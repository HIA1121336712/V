...
// 更新邻居表
void GpsrRouting::updateNeighborTable(const std::map<int, std::pair<double, double>>& vehiclePositions) {
    neighborTable.clear();  // 清空当前邻居表

    for (const auto& [vehicleId, position] : vehiclePositions) {
        if (vehicleId != my_id) {
            double distance = calculateDistance(my_x, my_y, static_cast<int>(position.first), static_cast<int>(position.second));
            if (distance <= 50.0) {  // 判断是否在50米范围内
                GPSR_neighborRecord neighbor;
                neighbor.id = vehicleId;
                neighbor.x = static_cast<int>(position.first);
                neighbor.y = static_cast<int>(position.second);
                neighbor.ts = simTime().dbl();  // 使用当前仿真时间作为时间戳
                neighborTable[vehicleId] = neighbor;
            }
        }
    }
}
